...
async function streamGemini({ cfg, model, messages, images, signal, onToken, retried = false }) {
  const system = messages.filter((item) => item.role === 'system').map((item) => item.content).join('\n\n');
  const contents = toGemini(messages.filter((item) => item.role !== 'system'), images);
  const url = geminiStreamUrl(cfg.baseUrl, model);
  const thinking = retried ? null : geminiThinking(model, cfg.thinking);
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      ...
      contents,
      generationConfig: geminiGenerationConfig(cfg, thinking),
    }),
    signal,
  });
  if (!response.ok) {
    const detail = sanitizeError(await response.text());
    if (!retried && thinking && response.status === 400 && /thinking/i.test(detail)) {
      return streamGemini({ cfg, model, messages, images, signal, onToken, retried: true });
    }
...
function geminiGenerationConfig(cfg, thinking) {
  const generationConfig = {
    temperature: 0.7,
    maxOutputTokens: thinking ? Math.min(8192, Math.max(cfg.maxTokens, 4096)) : cfg.maxTokens,
  };
...
function upstreamMessage(status, detail) {
  if (status === 401 || status === 403) {
    return 'The AI service rejected the server credentials. Check AI_API_KEY on the server. The key is not shown here.';
  }
  ...
  if (/timeout|timed out/i.test(detail)) return 'The AI service took too long. Your message is still here — tap Retry.';
  if (status === 0 && /fetch failed|network|ENOTFOUND|ECONNREFUSED/i.test(detail)) {
    return 'Can’t reach the AI service. Check the internet connection and try again.';
  }
  return 'Sana couldn’t complete that reply. Your message is still here — tap Retry.';
}
